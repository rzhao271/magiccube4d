 ...
 * </ul>
 * In a 3<sup>d</sup> puzzle, the face center stickers never move, and so when we refer to the "color" of a face, we mean the color of its center sticker. <br>
 * In a 2<sup>d</sup> puzzle, the solve algorithm never moves the first cubie, and the color of a face means the respective color on that cubie (or the opposite color in the solved puzzle).
 * <p>
 * That's it!
 * 
 * <hr>
...
public class NdSolve {
    ...
    private NdSolve() {} // non-instantiatable, only has static member functions
     ...
     * isSane}) or if there are any bits set in whichToCheckPositions which are not also set in whichToCheckOrientations. Currently also throws an Error if n > 3.
     */
    public static boolean isSolvable(String puzzleString,
        int whichToCheckPositions, int whichToCheckOrientations,
        java.io.PrintWriter progressWriter, int debugLevel)
     ...
     * <p>
     * Throws an Error if puzzleString is insane or unsolvable (see {@link #isSane isSane}, {@link #isSolvable isSolvable}) or if there are any bits set in whichToOrient which are not also set in whichToPosition. <br>
     * Currently also throws an Error if n > 3 (the solve algorithm is only implemented for n &le; 3).
     * <p>
     ...
     * <p>
     * Throws an Error if movesString does not represent a valid sequence of moves or if puzzleString is insane (see {@link #isSane isSane}). It doesn't matter whether puzzleString is solvable or not.
     */
     ...
     * 90-degree twists that can be applied to the given puzzle (see {@link #apply apply}).
     * <p>
     ...
     * <p>
     * Throws an Error if puzzleString is insane (see {@link #isSane isSane}) or if not 0 &le; minScrambleChen &le; maxScrambleChen or if d &lt; 3.
     * <p>
     ...
     * <p>
     * Has many command line options and interactive shell commands. For example, to scramble a 3<sup>5</sup> puzzle with 9 or 10 twists and then solve it:
     * 
     * <pre>
...
class Arrays {
    ...
    private Arrays() {} // non-instantiatable, only has static member functions
